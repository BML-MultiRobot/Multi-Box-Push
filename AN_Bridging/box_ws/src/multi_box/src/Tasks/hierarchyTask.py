...
class HierarchyTask(Task):
    def __init__(self, take_states=True):
        super(HierarchyTask, self).__init__(take_states)
    ...
    def postTraining(self):
        if self.mode == 'GET_STATE_DATA':
            self.data_to_txt(path =  '/home/jimmy/Documents/Research/AN_Bridging/' +self.primitive + '_state_data_GP2_better.txt')
            sys.exit(0)
        else:
