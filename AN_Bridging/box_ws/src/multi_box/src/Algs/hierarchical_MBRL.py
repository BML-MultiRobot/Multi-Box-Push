...
"""  Implementation for model-based guidance in model-free contexts. Will continue with this work another time."""
...
class Hierarchical_MBRL(Agent):
    ...
    def saveModel(self):
        torch.save(self.model.state_dict(), '/home/jimmy/Documents/Research/AN_Bridging/model_training_data/model_differential2.txt')
        self.policy.saveModel()
        pass
    
    def save_data(self, data):
        path = '/home/jimmy/Documents/Research/AN_Bridging/model_training_data/model_data_elevated_push_hole_differential_2.txt'
        with open(path, "wb") as fp:  
            pickle.dump(data, fp)
        return 
    
    def train_model_beforehand(self):
        path = '/home/jimmy/Documents/Research/AN_Bridging/model_training_data/Pure_Q/Combination_2/model_data_elevated_push_hole_differential_2.txt'
    ...
    def train_model(self):
        if self.trainMode and self.method == 'AIDED_MFRL':
            ...
            plt.plot(range(len(self.loss)), self.loss)
            plt.savefig( '/home/jimmy/Documents/Research/AN_Bridging/model_training_data/value_loss_update.jpg')

    def train(self, episode=0):
        if max(len(self.policy.exp), len(self.model.transitions)) >= 1000 and not self.testing_to_record_progress and self.trainMode:
